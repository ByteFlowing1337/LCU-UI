...
def _try_ban_champion(socketio, client, action_id, candidates, unavailable_ids):
    """尝试自动禁用英雄"""
    for cid in candidates:
        if not cid or cid in unavailable_ids:
            continue
            try:
                success = complete_action(client, action_id, cid, action_type='ban')
            if success:
        ...
        def complete_action(client, action_id, champion_id, action_type='pick'):
            ...
            action = client.get_champ_select_session()
            if not action:
                return False
